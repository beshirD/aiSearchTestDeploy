...
import { getLinks } from 'apiSdk/links';
...
function DashboardPage() {
  ...
  const botStat = useSWR<PaginatedInterface<BotInterface>>(
    () => `/bots`,
    () => getBots({ limit: 1, offset: 0 }),
  );

  const linkStat = useSWR<PaginatedInterface<LinkInterface>>(
    () => `/links`,
    () => getLinks({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Bots',
      result: botStat?.data?.totalCount,
      isLoading: botStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Links',
      result: linkStat?.data?.totalCount,
      isLoading: linkStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
